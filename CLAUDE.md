...
**实现方案：mDNS 优先 + HTTP 探测降级**

```
用户点击"扫描局域网"
  ↓
前端发送 WebSocket 消息 {"type": "scan_lan"}
  ↓
Go 服务器执行扫描：
  1. mDNS 查询 _airlink._tcp.local（1-2 秒）
  2. 如果失败或无结果，HTTP 探测当前子网 /24（并发请求）
  ↓
返回设备列表：[{name, ip, device_id}]
  ↓
前端显示设备，用户点击"连接"
```

**mDNS 服务注册：**
- 服务名称：`_airlink._tcp.local`
- 端口：用户配置的端口（默认 8080）
...
**房间生命周期：**
1. **创建**：Host 设备创建房间 → 生成 PIN → 显示等待界面
2. **加入**：其他设备输入 PIN → 验证成功 → 加入房间
...
discovery:
  ...
  http_probe_enabled: true       # 启用 HTTP 探测
  service_name: "_airlink._tcp"  # mDNS 服务名称
